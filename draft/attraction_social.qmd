...
![Simulations mixing frequency-based social influence (conformity) and content preference Each line represesnts a run, the thick black line is the average. Top panel: Proportion of population with trait A in the discrete traits condition. Top-left: only conformity acting ($\alpha=0$). Top-right: content preference and content preference ($\alpha=0.1$). For all simulations in the discrete traits condition, $T=50, D=1, p_0=0.5$ Bottom panel: Average trait value in the continuos traits condition. Bottom-left: only conformity acting ($\alpha=0$). Inset plot: the shaded area shows the variance for a respresentative run. Bottom-right: conformity and content preference ($\alpha=0.1$). For all simulations in the continuos traits condition, $T=100$. For all simulations $N=1000$, and 100 runs per condition. \label{fig1}](../figures/for_draft/fig1.pdf)

![Simulations mixing frequency-based social influence (conformity) and content preference. Each line represesnts a run, the thick black line is the average. Left panel: Proportion of population with trait A in the discrete traits condition. Conformity and content preference starting from a population with only trait B ($p_0=0$). Red lines: runs with $\alpha=0.1$ Blue lines: runs with $\alpha=0.2$. For all runs: $T=50, D=1$. Right panel: Average trait value in the continuos traits condition. Conformity and content preference starting from a population where all individuals are initialised with $p=0$. For all runs: $T=100, \alpha=0.1$ For all simulations $N=1000$, and 100 runs per condition. \label{fig2}](../figures/for_draft/fig2.pdf)
...
![Simulations mixing demonstrator-based social influence and content preference. Each line represesnts a run, the thick black line is the average. Top panel: Proportion of population with trait A in the discrete traits condition. Top-left: only demonstrator-based social influence acting ($\alpha=0$). Top-right: demonstrator-based social influence and content preference ($\alpha=0.1$). For all simulations in the discrete traits condition, $D=1$ and $p_0=0.5$ Bottom panel: Average trait value in the continuos traits condition. Bottom-left: only demonstrator-based social influence acting ($\alpha=0$). Bottom-right: demonstrator-based social influence and content preference ($\alpha=0.1$). For all simulations $N=1000$, $T=200$, $C_s=0.01$, $C_{copy}=5$, and 100 runs per condition. \label{fig4}](../figures/for_draft/fig4.pdf)
...
The results of the simulations show that, in virtually all scenarios, content preferences stabilise culture at the point where they are directed to. When social influence mechanisms act alone, they also bring traits to fixation, but which trait/trait configuration wins is left to chance. Social influence mechanisms---both based on frequency (conformity) and on demonstrators' features (e.g., prestige bias)---, are independent from traits' features, and non-directional, so that content preferences are the only directional forces in the system, and their existence is sufficient to determine the fate of the system. Only in one case, that is, when the majority of the population adopts the non-favoured trait in a discrete choice and conformity acts, the preference for the (minority) traits need to be sufficiently strong to overcome the initial majority (see Figure \ref{fig3}).

SIMILAR RESULTS IN OTHER PAPERS, INCLUDING [@yan2023]

Conceptually, it is important to think to social influence and preferences for content not as opposing forces. Non-directional social influence provide, so to say, strength, to the weak but directional preferences for content (as illustrated in Figure \ref{fig5}). In other words, culture magnifies individual-level tendencies, allowing them to become stable at population level. In the models presented here, only one preference was considered, but the same logic applies to more realistic situations with many different forces, and we would expect culture to homogenise population towards the stronger ones.

The obvious take home message of these results is that, if our question is why some things are culturally successful and others are not (as from the beginning of this paper), weak but stable non-social forces need to be taken into account. A possibly less obvious take home message concerns the interpretation of cross-cultural regularities.

human universals do not need strong cognitive (or others) predispositions.

Evolutionary psychologists have...While everyone would agree this is a false dichotomy, it is important to understand why it is so.

Limitations:

-   content preference is deterministic? simplification but it would not change qualitatively the results.

-   social influence detached from trait.
