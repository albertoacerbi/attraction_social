...
conformist_transmission_continuous <- function (N, alpha_attr, p_0, t_max, r_max) {
  ...
  for (r in 1:r_max) {
    ...
    for(t in 1:t_max){
      ...
      errors <- runif(N, min = -rowVars(demonstrators), max = +rowVars(demonstrators))
      conf_population <- rowMeans(demonstrators) + errors
      conf_population[conf_population > 1] = 1
      conf_population[conf_population < 0] = 0
